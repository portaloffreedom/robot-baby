...
import numpy as np
import random
from rlpower_controller import RLPowerController
...
class RLPowerAlgorithm:
    ...
    def __init__(self, config_parameters):
        ...
        self._current_spline = np.array([[0.5 + random.normalvariate(0, self.SIGMA) for x in range(self._spline_size)] for y in range(self.NUM_SERVOS)])
    ...
    def next_evaluation(self, controller):
        ...
        uniform = np.array([[random.normalvariate(0, self.SIGMA) for x in range(self._spline_size)] for y in range(self.NUM_SERVOS)])
    ...
    def get_current_fitness(self):
        ...
        if self._fitness_evaluation == 'manual':
            fitness = float(raw_input("Enter fitness of current gait: "))
